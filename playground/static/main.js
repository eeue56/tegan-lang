...
(() => {
  ...
  // ../src/generate.ts
  function joinChar(expression, next) {
  ...
  function generateExpression(expression) {
    switch (expression.kind) {
      case "PushToStackExpression": {
        const maybeNamed = expression.namedStack === null ? "" : ` => ${expression.namedStack}`;
      ...
      case "LocalBlockExpression": {
        const lengthOfLongestExpression = expression.values.slice(0, expression.values.length).filter((exp) => exp.kind !== "CommentExpression").map(generateExpression).sort((a, b) => a.length - b.length).reverse()[0]?.length || 0;
        const base2 = `${generateExpression(expression.baseCall).trim()} <-`;
        const baseLength = base2.length;
        const indentSize = 4;
        ...
        const output2 = [`
${base2}${extraBaseDash}`];
        if (expression.values[0] && expression.values[0].kind === "CommentExpression") {
          output2.push(" ");
        } else {
          output2.push("\n");
        }
        ...
        for (const subExpression of expression.values) {
          ...
          if (subExpression.kind !== "CommentExpression") {
            output2.push(" ".repeat(indentSize));
          }
          output2.push(generateExpression(subExpression));
          output2.push(joinChar(subExpression, next));
          i++;
        }
        output2.push(`^-${extraTrailDash}
`);
        return output2.join("");
      }
      case "TeganNumberExpression": {
  ...
  function generateTegan(program) {
    const output2 = [];
    let i = 0;
    for (const expression of program.expressions) {
      const next = program.expressions[i + 1];
      output2.push(generateExpression(expression));
      output2.push(joinChar(expression, next));
      i++;
    }
    return output2.join("");
  }
  ...
  function isStringQuoteChar(char) {
    return char === '"';
  }
  function isIntoChar(char) {
    return char === "<" || char === "-";
  }
  function isOutroChar(char) {
    return char === "^" || char === "-";
  }
  function isNumberChar(char) {
    return ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"].includes(char);
  }
  function isCommentBeginChar(char) {
    return char === "%";
  }
  function isConditionalChar(char) {
    return char === "?";
  }
  function isUseChar(char) {
    return char === "@";
  }
  function isLabelChar(char) {
    return char === ">" || char === "=";
  }
  function tokenize(str) {
    let state = { kind: "ReadyForNextToken" };
    const tokens = [];
      ...
      switch (currentState.kind) {
        ...
        case "ReadingWhitespace": {
          tokens.push({
            ...
            startIndex: currentState.startIndex,
            endIndex: currentState.startIndex + currentState.whitespaceSoFar.length
          });
          break;
        }
        case "ReadingComment": {
          tokens.push({
            ...
            startIndex: currentState.startIndex,
            endIndex: currentState.startIndex + currentState.commentSoFar.length + 1
          });
          break;
        }
        case "ReadingString": {
          tokens.push({
            ...
            startIndex: currentState.startIndex,
            endIndex: currentState.startIndex + currentState.stringSoFar.length + 2
          });
          break;
        }
      }
  ...
  function expectOne(kind, token) {
    ...
    return { kind: "Ok", tokensConsumed: 1, value: token };
  }
  function parseNumber(tokens) {
    const number2 = expectOne(
      "NumberToken",
      tokens[0]
    );
    switch (number2.kind) {
      ...
      case "Ok": {
        return {
          ...
          value: {
            ...
            value: number2.value.value
          },
          tokensConsumed: number2.tokensConsumed
        };
      }
    }
  }
  ...
  function values(...results) {
    return results.map(
      (arg) => arg.kind === "Ok" ? arg.value : void 0
    );
  }
  function oneOf(tokens, ...parsers) {
    const errorMessages = [];
  ...
  function parseLabel(tokens) {
    ...
    const name2 = expectOne("NameToken", tokens[1]);
    const errors = hasErrors(labelToken, name2);
    if (errors) {
      return {
        kind: "Err",
        message: errors,
    ...
    const [_, label] = values(labelToken, name2);
    return {
      kind: "Ok",
      value: label,
      tokensConsumed: 2
  ...
  function parsePushToStack(tokens) {
    const push = expectOne(
      "PushToStackToken",
      tokens[0]
    );
    const number2 = parseNumber([tokens[1]]);
    const string2 = parseString([tokens[1]]);
    if (hasErrors(number2) && hasErrors(string2)) {
      return {
        kind: "Err",
    ...
    const numberOrString = hasErrors(number2) ? string2 : number2;
  ...
  function parseFunctionCall(tokens) {
    ...
    const name2 = expectOne("NameToken", tokens[0]);
    const errors = hasErrors(name2);
    if (errors) {
      return {
        kind: "Err",
  ...
  function parseInto(tokens) {
    ...
    const errors = hasErrors(into, functionCall, outro) || miniProgram.errors.map((err) => err.message).join("\n");
    if (errors) {
      return {
        kind: "Err",
    ...
    const [call] = values(functionCall);
    return {
      kind: "Ok",
      value: {
  ...
  function parseString(tokens) {
    ...
    const [string2] = values(maybeString);
    return {
      kind: "Ok",
      value: {
  ...
  function parseConditional(tokens) {
    ...
    const [_, subExpression] = values(conditional, namePushOrUse);
    const subLength = namePushOrUse.kind === "Ok" ? namePushOrUse.tokensConsumed : 0;
    const maybeLabel = parseLabel(tokens.slice(2, tokens.length));
    const label = maybeLabel.kind === "Ok" ? maybeLabel.value.value : null;
    const labelLength = maybeLabel.kind === "Ok" ? maybeLabel.tokensConsumed : 0;
    return {
  ...
  function parseUse(tokens) {
    const use = expectOne("UseToken", tokens[0]);
    const name2 = parseFunctionCall(tokens.slice(1, tokens.length));
    const errors = hasErrors(use, name2);
    if (errors) {
      return {
        kind: "Err",
        message: `Failed to parse conditional block: ${errors}`,
  ...
  function parseComment(tokens) {
    ...
    const [comment2] = values(commentResult);
    return {
      kind: "Ok",
      value: {
  ...
  function parseExpressions(tokens) {
    ...
    for (let i = 0; i < tokens.length; ) {
      ...
      previousIndex = i;
      let parserToUse = PARSERS[token.kind];
      let minTokens = MIN_TOKENS[token.kind];
  ...
  function inlineErrors(str, errors) {
    const underline2 = "\x1B[4;31m";
    const red = "\x1B[33m";
    const normalCode = "\x1B[0m";
    const reversedErrors = errors.slice(0, errors.length).sort((first, second) => first.regionStart - second.regionStart).reverse();
    for (const error of reversedErrors) {
      ...
      before = before.slice(0, error.regionStart) + red + underline2 + before.slice(error.regionStart, error.regionEnd) + normalCode + before.slice(error.regionEnd, before.length);
      str = before + ` <= ${red}${error.message}${normalCode}` + str.slice(insertIndex, str.length);
    }
    return str;
  }
  function parseProgram(str) {
    ...
    const program = parseExpressions(
      tokens.filter(
        (token) => token.kind !== "WhitespaceToken" && token.kind !== "CommentToken" && token.kind !== "NewlineToken"
      )
    );
    if (program.errors) {
  ...
  function parseProgramWithComments(str) {
    const tokens = tokenize(str);
    const program = parseExpressions(
      tokens.filter(
        (token) => token.kind !== "WhitespaceToken" && token.kind !== "NewlineToken"
      )
    );
    if (program.errors) {
      program.overallError = inlineErrors(str, program.errors);
  ...
  var MIN_TOKENS, PARSERS, MIN_TOKENS_BY_PARSER_NAME;
  var init_parser = __esm({
    "../src/parser.ts"() {
      ...
      MIN_TOKENS_BY_PARSER_NAME = Object.fromEntries(
        Object.entries(MIN_TOKENS).map(
          ([token, count]) => {
  ...
  async function debugRepl(initialState, program, currentExpressionIndex) {
    let currentBuffer = [];
    const rl = readline.createInterface({
      input: import_process.stdin,
    ...
    for await (const line of rl) {
      if (line.trim() === "") {
        ...
        if (program2.errors.length > 0) {
          console.log(`Errors while parsing: ${program2.errors.join("\n")}`);
          ...
          linesAddedToCurrentBufferSinceLastParsing = [];
          state = await runProgramFromState(program2, state);
        }
  ...
  async function repl(initialState) {
    let currentBuffer = [];
    const rl = readline.createInterface({
      input: import_process.stdin,
      output: import_process.stdout,
  ...
  var import_process, readline, DEBUG_STRINGS;
  var init_repl = __esm({
    "../src/repl.ts"() {
      "use strict";
      import_process = __require("process");
      readline = __toESM(__require("readline"));
      init_generate();
      init_parser();
      init_runtime();
      DEBUG_STRINGS = [":quit", ":help", ":next", ":continue", ":quit"];
    }
  });

  // ../src/runtime.ts
  function flattenUseAndStack(state) {
  ...
  async function debug(state, program, currentExpressionIndex) {
    if (typeof process !== "undefined") {
      let debugRepl2 = (await Promise.resolve().then(() => (init_repl(), repl_exports))).debugRepl;
      return {
        ...await debugRepl2(state, program, currentExpressionIndex),
        kind: "DebugState"
      };
    }
    return state;
  }
  async function evaluateExpression(expression, program, currentExpressionIndex, state) {
    switch (expression.kind) {
      case "PushToStackExpression": {
      ...
      case "FunctionCallExpression": {
        const localStack = expression.namedStack ? state.namedStacks[expression.namedStack] || [] : state.stack;
        const localState = { ...state, stack: localStack };
      ...
      case "LocalBlockExpression": {
        ...
        for (let localExpressionIndex = 0; localExpressionIndex < expression.values.length; localExpressionIndex++) {
      ...
      case "UseExpression": {
        ...
        if (originalStack.length > 0) {
          const value = originalStack.pop();
          const result = await evaluateExpression(
            expression.functionName,
            program,
            currentExpressionIndex,
            { ...state, stack, using: [value] }
          );
          result.using = [];
  ...
  async function runProgramFromState(program, state) {
    let i = 0;
    for (const expression of program.expressions) {
      state = await evaluateExpression(expression, program, i, state);
      i++;
  ...
  var WidgetView = class _WidgetView extends ContentView {
    ...
    get overrideDOMText() {
      ...
      while (top3.parent)
        top3 = top3.parent;
      let { view } = top3, text = view && view.state.doc, start = this.posAtStart;
  ...
  function rectanglesForRange(view, className, range) {
    ...
    function piece(left, top3, right, bottom) {
      return new RectangleMarker(className, left - base2.left, top3 - base2.top, right - left, bottom - top3);
    }
  ...
  init_parser();
  init_runtime();
  function viewStack(name2, stack) {
    return `
<div class="stack">
    <h2>${name2}</h2>
    ${stack.map((item) => `<div class="stack-item">${item.value}</div>`).join("")}
</div>
`;
  }
  function main() {
    ...
    document.getElementById("format-program")?.addEventListener("click", () => {
      ...
      const output2 = generateTegan(program);
      view.dispatch({
        changes: {
          from: 0,
    ...
    const lintExtension = linter((view2) => {
    ...
    const view = new EditorView({
      parent: root,
      doc: storedProgram,
      extensions: [basicSetup, lintExtension, updateListener2]
    });
  }
  main();
})();
